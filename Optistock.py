...
class Import:
    ...
    def view_imports(self):
        ...
        print(f"{'Product Name':<20} | {'Quantity(Unit)':<30}| {'Buying Price(FRW)':<20} | {'Total Buying Price(FRW)':<25} | {'Import Date':<20}")
        print("-" * 120)
        for row in rows:
    ...
    def update_existing_prodcut(self, product_name, quantity, unit, buying_price, selling_price, import_date):
        ...
        new_total_buying_price = new_quantity * new_buying_price

        cursor.execute('''UPDATE stocked_products SET product_name = %s, quantity = %s, unit = %s, buying_price = %s, total_buying_price = %s, selling_price = %s WHERE product_name = %s''', (product_name, new_quantity, new_unity, new_buying_price, new_total_buying_price, new_selling_price, product_name))
...
class Stock:
    ...
    def update_stock(self, product_name, new_product_name, quantity, unit, buying_price, selling_price):
        cursor = self.connection.cursor()
        total_buying_price = quantity * buying_price

        cursor.execute('''UPDATE stocked_products SET product_name = %s, quantity = %s, unit = %s, buying_price = %s, total_buying_price = %s, selling_price = %s WHERE product_name = %s''',
        (new_product_name, quantity, unit, buying_price, total_buying_price, selling_price, product_name))
        self.connection.commit()
    ...
    try:
        ...
        if choice == 1:
            ...
            while True:
                ...
                print("3. Import Existing Product(s) In Your Stock")
                print("4. Delete Option for Imported products")
                ...
                try:
                    ...
                    if choose == 1:
                        ...
                        print("\n", " " * 20, "____IMPORTING EXISTING PRODUCT(S) IN STOCK____\n")
                        import_times = int(input("How many products do you want to import?:  "))
                        print()
                        for count in range(import_times):
                            product_name = input(f"Enter the name of your product_{count + 1}:  ")
                            ...
                            if result:
                                ...
                                manage_import.update_existing_prodcut(product_name, quantity, unit, buying_price, selling_price, import_date)
                            else:
                                print(f"\n____{product_name} is not found in your stock, please check your imports!____")
                                break
                        print()
                        cursor.close()
            ...
            while True:
                ...
                try:
                    ...
                    if choose == 1:
                        ...
                        if result:
                            new_product_name = input(f"Enter the new name for {product_name}:  ")
                            ...
                            while True:
                                ...
                                print("3. Pieces")
                                try:
                                    unit_choice = int(input(f"Update the unit of your product({new_product_name}): N°_"))
                                    if unit_choice == 1:
                                        unit = "KG"
                                        break
                                    elif unit_choice == 2:
                            ...
                            buying_price = int(input(f"Enter the new buying price per product({new_product_name}):  "))
                            selling_price = int(input(f"Enter the new selling price per product({new_product_name}):  "))

                            manage_stock.update_stock(product_name, new_product_name, quantity, unit, buying_price, selling_price)
                        else:
                            print(f"\n____{product_name} is not found in your stock!____\n")
                        cursor.close()
